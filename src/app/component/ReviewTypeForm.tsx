...
interface ReviewTypeFormProps {
  onSuccess: () => void;
}

export default function ReviewTypeForm({ onSuccess }: ReviewTypeFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === 201) {
        setSuccess("Review type added successfully!");
        setName("");
        onSuccess();
      }
    } catch (err: any) {
  ...
  return (
    <>
      <input type="checkbox" id="review-toggle" className="hidden peer" />
